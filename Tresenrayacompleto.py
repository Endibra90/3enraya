...
class Jugador:
  ...
  def __str__(self):
    return (str(self.__nombre) + " " + str(self.__tipo_ficha) + " "+ str(self.__puntuacion))
...
class Tablero:
  ...
  def __str__(self):
    ...
    for linea in self.__matriz_celdas:
      for celda in linea:
        salida = salida + celda.get_tipo_ficha2() + " "
      salida = salida + "\n"
    return (salida)
  ...
  def reiniciar_tablero(self):
    ...
    for i in range(self.__alto):
      for j in range(self.__ancho):
        self.__matriz_celdas[i][j].reiniciar_celda() 
  ...
  def hay_tres_en_raya(self):
    ...
    for posible in posibles:
      ...
      for cordenada in coordenadas:
        fichas.append(self.__matriz_celdas[int(cordenada[0])][int(cordenada[1])].get_tipo_ficha2())
    ...
    if (isinstance(jugador,Jugador)):
      ...
      while (not termina and intentos < numer_max_intentos):
        ...
        try:
          ...
          print("Mete valores entre 0 y 2")
        except y:
          print("Esta mal el valor.")
        finally:
          intentos = intentos + 1
